...
Would not recommend trying to run on a standard desktop, `RCH` is a good choice.
...
**OQ Engine Version**
Version 3.23.2 works correctly, there were some issues with earlier versions so would not recommend using those.
...
```ini
[general]
description = Hazard curves,Granular configs GSIM LT final EE
random_seed = 25
...
The only options that should be changed are:
 - `site_model_file`, `source_model_logic_tree_file` and `gsim_logic_tree_file`  if those files have different names or location
 - `intensity_measure_types_and_levels` to change the IM levels for which hazard is calculated. Note that increasing the number of IM levels will increase run time and memory consumption
...
I tend to symlink (`ln -s`) the datastore into the run-directoy so I know which `calc_id` belongs to each run.
...
This is done via the `nshm_2022_cmds` script in `scripts` folder.
...
### Extraction
Extraction of the disaggregation values is again done via the `nshm_2022_cmds.py` script using the `extract-disagg` command

```
 Usage: nshm_2022_cmds.py extract-disagg [OPTIONS] CALC_ID OUTPUT_DIR
...
╭─ Arguments ───────────────────────────────────────────────────────────────────────────────────────────╮
│ *    calc_id         INTEGER  OpenQuake calculation ID [default: None] [required]                     │
│ *    output_dir      PATH     Directory to save the extracted disaggregation data [default: None]     │
│                               [required]                                                              │
╰───────────────────────────────────────────────────────────────────────────────────────────────────────╯
╭─ Options ─────────────────────────────────────────────────────────────────────────────────────────────╮
│ --disagg-kind        TEXT  Kind of disaggregation to extract [default: TRT_Mag_Dist_Eps]              │
│ --help                     Show this message and exit.                                                │
╰───────────────────────────────────────────────────────────────────────────────────────────────────────╯
```

### 3D Disaggregation Plot
Generation of disaggregation 3D bar plots can be done using the `nshm_2022_cmds.py` script using the `disagg-plot` command
![Disaggregation Plot](disagg_chch_nshm.png)